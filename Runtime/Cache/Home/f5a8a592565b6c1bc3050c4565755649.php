...
$(function(){
	...
	//alert(r[0].id);	
		...
		$.each(r, function(i, content){
			var key = content.Subject;	
			var kwgObj = $(".keyWordGroup:eq("+i+")");
			//alert(key);
	
			kwgObj.find(".keyword").val(key);
		
			if((content.District)&&(content.Ex_subect)&&(content.type)){
				
				kwgObj.attr('id', content.id);
	...
	$(".seniorset").click(function(){
		window.location.href = "/weixin/index.php/Home/Order/push";
	});
	$(".set2,.set3").click(function(){
		...
		window.location.href = "/weixin/index.php/Home/Order/details?index="+index+"&keyword="+escape(value)+"&id="+id;
	});
...
function copyText(){
	...
	var param = {
		...
		$.ajax({
			type: "POST",
			url: "/weixin/index.php/Home/Order/addate",
			data:param,
			dataType: "json",
...
<body class="keysetpage">
	<div class="header">
		...
		<span class="complate hide" onclick="copyText()">完成</span>
