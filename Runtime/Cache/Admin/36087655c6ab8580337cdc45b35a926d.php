        ...
        function checkall(){ //jquery获取复选框值
            ...
            if(chk_value.length==0){
                ...
                if(confirm('确定现在开始推送吗?')){
                    $("#pushall").html('请稍后！');
                    var url = "<?php echo U('Home/Index/sendMore');?>";
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
                        ...
                        <?php if(($vo["typename"]) != ""): echo ($vo["typename"]); ?>
                            <?php else: ?>
                            <a href="<?php echo U('edit?id='.$vo['id']);?>" style="color:red;">去添加</a><?php endif; ?>
                        </span>
                    </td>
                    <th class="" width="10%"><?php echo ($vo["source"]); ?></th>
                    <th class="" width="5%"><a href="javascript:;" onclick="sendone('<?php echo ($vo["id"]); ?>')" id="sendone<?php echo ($vo["id"]); ?>">推送</a></th>
                    <th class="" width="5%"><?php echo ($vo["times"]); ?></th>
                    <td>
                        <a href="<?php echo U('edit?id='.$vo['id']);?>" >编辑</a>
                        <a href="javascript:void(0);" class="confirm " onclick="delnews('<?php echo ($vo["id"]); ?>')">删除</a>
                    </td>
