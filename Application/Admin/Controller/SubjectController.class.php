...
class SubjectController extends AdminController
{
    ...
    public function save()
    {
        ...
        {
            if($v) {
                $exist = $model -> where(" title = '$v' ")->find();
                if(!$exist) {
                    $data = array('title' => $v, 'addtime' => time());
                    $model->add($data);
                }
            }
        }
    ...
    {
        $id = I('post.id');
        $res = M('subject')->where('id = '.$id)->save(array('status' => 0));
        if ($res)
        {//添加成功
            echo '1';
        } else {//修改失败
    ...
    public function update()
    {
        if(I('post.id') && I('post.tags'))
        {
            ...
            $res = M('subject')->where('id = '.$id)->save($data);
            if ($res)
            {//添加成功
    ...
    {
        $id = I('get.id');
        $exist = M('subject')->where(" id= $id ")->find();
        if($id && $exist) {
