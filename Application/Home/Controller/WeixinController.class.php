...
class WeixinController extends HomeController
{
		...
		else
		{
			if(is_string($id)) {//单条
				...
				foreach ($res as $k => $v) {
					...
					$content =substr($v['clean_content'],0,150).'......';
					...
					//file_put_contents(APP_PATH.'Home/Controller/log.txt','-pic-:'. $pic .'---',FILE_APPEND );
	...
	public function  sendMessageduo($openid,$token, $res ,$tem,$cot)
	{
		...
		foreach ($res as $k1 => $v1) {
			...
			$description=$v1['clean_cotent'];
			$arr2[$pic]=urlencode($pic);
			$arr2[description]=urlencode($description);
